...
'''
Compare two lists of pits detected by DCC from different exposures (front and back).
'''
...
def plot(list1, list2, name1, name2, save=False):
    ...
    if save:
        plt.savefig(f"plots/DCC_pr_alignment_{name1}_and_{name2}.png", dpi=100)
    plt.show()


def compare_DCC_aligned(tolerance=12):
    """
    Compares pits detected by the DCC model from front and back exposures, identifying common, only-front, and only-back pits.
    ...
    front_eval = mhio.load_json("predicted_etch_pits/DCC/final_pr.txt")
    back_eval = mhio.load_json("predicted_etch_pits/DCC/final_reverse_pr.txt")
    ...
    mhio.save_json(only_front, "predicted_list/DCC/Aligned_Front_Not_back.txt")
    mhio.save_json(only_back, "predicted_list/DCC/Aligned_Back_Not_Front.txt")
    mhio.save_json(common, "predicted_etch_pits/DCC/aligned.txt")
