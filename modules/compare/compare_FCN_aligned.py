...
'''
Compare two lists of pits detected by the FCN model from different exposures (front and back).
'''


def plot(list1, list2, name1, name2, save=False):
    """
    Plots two sets of pits (from different methods or exposures) and compares them.

    Parameters:
    - list1, list2: Lists containing pit coordinates from two methods/exposures.
    ...
    if save:
        plt.savefig(f"plots/FCN_pr_alignment_{name1}_and_{name2}.png", dpi=100)
    plt.show()


def compare_FCN_aligned(tolerance=12):
    """
    Compares pits detected by the FCN model from front and back exposures, identifying common, only-front, and only-back pits.
    ...
    front_eval = mhio.load_json("predicted_etch_pits/FCN/final_pr.txt")
    back_eval = mhio.load_json("predicted_etch_pits/FCN/final_reverse_pr.txt")
    ...
    # Save the pits to their respective JSON files
    mhio.save_json(only_front, "predicted_list/FCN/Aligned_Front_Not_back.txt")
    mhio.save_json(only_back, "predicted_list/FCN/Aligned_Back_Not_Front.txt")
    mhio.save_json(common, "predicted_etch_pits/FCN/aligned.txt")
