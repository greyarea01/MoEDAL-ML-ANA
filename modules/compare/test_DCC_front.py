...
def evaluate_DCC_front(tolerance=15):
    """
    Compares predicted pits from DCC model (front exposure) with true pits, identifies false negatives,
    ...
    # Load predicted pits (front exposure) and true pits (aligned clean)
    predicted_list = mhio.load_json("predicted_etch_pits/DCC/final_pr.txt")
    ...
    mhio.save_json(false_pos, "predicted_list/DCC/front/final_false_pos.txt")
    mhio.save_json(false_neg, "predicted_list/DCC/front/final_false_neg.txt")
    mhio.save_json(true_pos, "predicted_list/DCC/front/final_true_pos.txt")
    mhio.save_json(frag, "predicted_list/DCC/front/final_fragments.txt")
    mhio.save_json(predicted_list_eval, "predicted_list/DCC/front/final_pred_list.txt")

    # Plot the comparison of true vs predicted pits
    plot(C1_true_eval, predicted_list_eval, "compare_front_DCC", True)
