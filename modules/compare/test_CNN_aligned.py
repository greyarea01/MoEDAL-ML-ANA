...
def evaluate_CNN_aligned(tolerance=15):
    """
    Compares predicted pits from the CNN model (aligned data) with true pits, identifies false negatives,
    ...
    predicted_list = mhio.load_json("predicted_etch_pits/CNN/aligned.txt")
    ...
    mhio.save_json(false_pos, "predicted_list/CNN/aligned/final_false_pos.txt")
    mhio.save_json(false_neg, "predicted_list/CNN/aligned/final_false_neg.txt")
    mhio.save_json(true_pos, "predicted_list/CNN/aligned/final_true_pos.txt")
    mhio.save_json(frag, "predicted_list/CNN/aligned/final_fragments.txt")
    mhio.save_json(predicted_list_eval, "predicted_list/CNN/aligned/final_pred_list.txt")

    # Plot the comparison of true vs predicted pits
    plot(C1_true_eval, predicted_list_eval, "compare_aligned_CNN", True)


# If the script is executed as a standalone program, run the evaluation
if __name__ == "__main__":
    evaluate_CNN_aligned()
