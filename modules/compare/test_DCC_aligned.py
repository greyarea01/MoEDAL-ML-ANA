...
def plot(true, predicted, name, save=False):
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend((tr, pr), ('True pits', 'Predicted pits_DCC'), scatterpoints=1, loc=0, ncol=2, fontsize=8)
    if save:
        plt.savefig("plots/" + name, dpi=100)
    plt.show()


def evaluate_DCC_aligned(tolerance=15):
    """
    Compares predicted pits from DCC model (aligned data) with true pits, identifies false negatives,
    ...
    predicted_list = mhio.load_json("predicted_etch_pits/DCC/aligned.txt")
    ...
    print("Comparing predicted pits (aligned) with true pits...")
    ...
    mhio.save_json(false_pos, "predicted_list/DCC/aligned/final_false_pos.txt")
    mhio.save_json(false_neg, "predicted_list/DCC/aligned/final_false_neg.txt")
    mhio.save_json(true_pos, "predicted_list/DCC/aligned/final_true_pos.txt")
    mhio.save_json(frag, "predicted_list/DCC/aligned/final_fragments.txt")
    mhio.save_json(predicted_list_eval, "predicted_list/DCC/aligned/final_pred_list.txt")

    # Plot the comparison of true vs predicted pits
    plot(C1_true_eval, predicted_list_eval, "compare_final_DCC_aligned", True)


# If the script is executed as a standalone program, run the evaluation
if __name__ == "__main__":
    evaluate_DCC_aligned()
